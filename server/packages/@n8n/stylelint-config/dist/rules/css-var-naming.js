...
const messages = stylelint.utils.ruleMessages(ruleName, {
    rejected: (variable, reason) => `Invalid CSS variable "${variable}": ${reason}`,
});
const meta = {
...
function validateCssVariable(variable) {
    ...
    if (!BASIC_PATTERN.test(variable)) {
        return {
            valid: false,
            reason: 'Must follow pattern: --[group]--[group]--... with lowercase alphanumerics and single dash within groups',
        };
    }
    ...
    if (absolutePropertyIndex + 1 < groups.length) {
        ...
        const isModifier = VARIANTS.has(valueGroup) ||
        ...
        if (!isModifier) {
            const isValidValue = SEMANTIC_VALUES.has(valueGroup) ||
    ...
    if (hasVariant && hasState) {
        ...
        if (variantIndex > stateIndex) {
            return {
                valid: false,
                reason: 'Variant should come before state (e.g., --button--background--primary--solid--hover)',
            };
        }
    }
    return { valid: true };
