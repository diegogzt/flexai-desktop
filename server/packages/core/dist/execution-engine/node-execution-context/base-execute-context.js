...
class BaseExecuteContext extends node_execution_context_1.NodeExecutionContext {
    ...
    async executeWorkflow(workflowInfo, inputData, parentCallbackManager, options) {
        ...
        const data = await this.binaryDataService.duplicateBinaryData(utils_1.FileLocation.ofExecution(this.workflow.id, this.additionalData.executionId), result.data);
        return { ...result, data };
    }
