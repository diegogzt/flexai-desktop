...
let License = class License {
    ...
    async init({ forceRecreate = false, isCli = false, } = {}) {
        this.logger.debug('FlexAI License initialized (Unlocked)');
    }
    async loadCertStr() {
    ...
    getMainPlan() {
        return {
            id: 'flexai-enterprise',
            productId: 'flexai-enterprise',
            validFrom: new Date(),
            validTo: new Date(new Date().setFullYear(new Date().getFullYear() + 100)),
            productMetadata: {
                terms: {
                    isMainPlan: true,
                },
