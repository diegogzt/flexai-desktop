...
let WorkflowBuilderService = class WorkflowBuilderService {
    ...
    async getService() {
        if (!this.service) {
            ...
            this.service = new ai_workflow_builder_1.AiWorkflowBuilderService(nodeTypeDescriptions, client, this.logger, this.instanceSettings.instanceId, this.urlService.getInstanceBaseUrl(), onCreditsUpdated, onTelemetryEvent);
        }
        return this.service;
    }
    ...
    async validateConfig(provider, apiKey) {
        try {
            const model = await ai_workflow_builder_1.AiWorkflowBuilderService.getModel({ apiKey, provider });
