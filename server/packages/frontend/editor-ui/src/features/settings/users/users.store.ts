...
import * as aiApi from '@n8n/rest-api-client/api/ai';
...
export const useUsersStore = defineStore(STORES.USERS, () => {
	...
	const submitContactEmail = async () => {
		await onboardingApi.submitEmailOnSignup(
	...
	const validateAiConfig = async (config: { provider: string; apiKey: string }) => {
		await aiApi.validateAiConfig(rootStore.restApiContext, config);
	};
