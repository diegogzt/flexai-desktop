...
watch(
	() => currentUser.value,
	(user) => {
		if (user?.settings?.aiConfig) {
			aiConfig.value = { ...user.settings.aiConfig };
		}
	},
	{ immediate: true },
);

const hasAiConfigChanges = computed(() => {
	const current = currentUser.value?.settings?.aiConfig;
	return (
		aiConfig.value.provider !== current?.provider || aiConfig.value.apiKey !== current?.apiKey
	);
});
...
const hasAnyChanges = computed(() => {
	return (
		hasAnyBasicInfoChanges.value ||
		hasAnyPersonalisationChanges.value ||
		hasAiConfigChanges.value
	);
});
...
async function validateAiConfig() {
	try {
		await usersStore.validateAiConfig(aiConfig.value);
		showToast({
			title: 'Configuration Valid',
			message: 'Your AI configuration is valid.',
			type: 'success',
		});
	} catch (e) {
		showError(e, 'Configuration Invalid');
	}
}

async function updateAiConfig() {
	if (!hasAiConfigChanges.value) return;
...
<template>
	<div :class="$style.container" data-test-id="personal-settings-container">
		...
		<div>
			<div class="mb-s">
				<N8nHeading size="large">AI Configuration</N8nHeading>
			</div>
			<div class="mb-s">
				<N8nInputLabel label="Provider">
					<N8nSelect v-model="aiConfig.provider" size="small">
						<N8nOption label="OpenAI" value="openai" />
						<N8nOption label="Anthropic" value="anthropic" />
					</N8nSelect>
				</N8nInputLabel>
			</div>
			<div class="mb-s">
				<N8nInputLabel label="API Key">
					<N8nInput
			...
			<div class="mb-s">
				<N8nButton label="Validate" @click="validateAiConfig" />
			</div>
		</div>
		<div>
			<N8nButton
